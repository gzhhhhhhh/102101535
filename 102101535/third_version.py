...
def get_bvid(page_number, number):  # 根据搜索页api获取视频的bvid号
    ...
    url = 'https://api.bilibili.com/x/web-interface/search/all/v2?' \
          'page=' + str(
           page_number) + '&keyword=%E6%97%A5%E6%9C%AC%E6%A0%B8%E6%B1%A1%E6%9F%93%E6%B0%B4%E6%8E%92%E6%B5%B7'
...
def generate_wordcloud():
    ...
    excel_file = "统计弹幕出现次数.xlsx"  # 读取Excel文件
    df = pd.read_excel(excel_file)

    text_data = " ".join(str(row['弹幕']) for _, row in df.iterrows())  # 将所有文本数据合并为一个字符串

    pic = np.array(Image.open('aa.jpeg'))  # 将图片转为数组格式

    wd = wordcloud.WordCloud(
        mask=pic,
        font_path='simhei.ttf',  # 字体替换
        background_color='white',  # 背景颜色
        contour_color='steelblue',  # 添加轮廓线颜色
        contour_width=2,  # 调整轮廓线宽度
        max_words=2000
    )  # 生成词云对象

    wd.generate(text_data)  # 生成词云图

    plt.figure(figsize=(8, 6))  # 调整画布大小，高为6，宽为12

    image_colors = wordcloud.ImageColorGenerator(pic)  # 图片颜色渲染词云图的颜色，用color_func指定
    plt.imshow(wd.recolor(color_func=image_colors), interpolation='bilinear')  # 显示已经重新着色的词云图

    plt.axis('off')  # 关闭显示x轴、y轴下标
    plt.tight_layout(pad=0.5)  # 去除紧凑布局的边缘
    plt.show()
